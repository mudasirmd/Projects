...
def test_mail_is_spam_or_not(mailBody):
    ...
    g = sns.FacetGrid(make_tidy(sample_space, train_scores, valid_scores), hue='Scores', height=5)
    ...
    pd.Series(clf.coef_.T.ravel(), index=vectorizer.get_feature_names_out()).sort_values(ascending=False)[:20]
